...
// This will be reset from i2c.c
uint8_t i2c_tx_buf_ptr;
...
ISR(TWI_vect)
{
	...
	case TW_ST_DATA_NACK:	//Master answered with NACK on last transmit
		if(i2c_tx_buf_ptr == i2c_msg_size)
		{
			i2c_status_register.last_tx_ok = 1;
			i2c_status_register.tx_data_in_buf = 0;
			i2c_msg_size = 0;
		}
		else
		{
	...
	// Stop / Repeated Start:
	case TW_SR_STOP:
		if (i2c_status_register.rx_data_in_buf == 1 && i2c_status_register.tx_data_in_buf && i2c_msg_size > 0)
		{
	...
	case TW_SR_GCALL_DATA_NACK:
		// Add the last received byte to the buffer as well
		i2c_rx_buf[i2c_rx_buf_ptr++] = TWDR;
