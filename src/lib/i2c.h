...
typedef union i2c_status_u
{
    ...
    struct
    {
        unsigned char last_rx_ok:1;		///< 1: last rx was successful
        unsigned char last_tx_ok:1; 	///< 1: last tx was successful
        unsigned char rx_data_in_buf:1;	///< 1: Received data is ready for reading
        unsigned char tx_data_in_buf:1; ///< 1: Data still present in tx buffer 0: data was sent/no data present
        unsigned char gen_call_rx:1;	///< 1: Data is from general call 0: individual address
        unsigned char unused_bits:3;
    };
} u_i2c_status;
...
/**
 * Set the data to be transmitted via i2c. This replaces any data waiting for transmit.
 *
